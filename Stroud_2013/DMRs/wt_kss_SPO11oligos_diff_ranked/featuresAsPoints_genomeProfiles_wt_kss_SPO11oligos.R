...
# Usage:
# ./featuresAsPoints_genomeProfiles_wt_kss_SPO11oligos.R kss_hypoCHG_DMRs_wt_kss_SPO11oligos_diff_ordered.txt kss_hypoCHG_DMRs 'HypoCHG DMRs in kyp suvh5 suvh6'
...
dat1Vdat2GenomePointPlot <- function(xplot,
  ...
  mtext(side = 2, line = 2.30, cex = 1.5,
        text = expression("Log"[2]*"(SPO11-1/gDNA)"))
...
# Plot
pdf(paste0(plotDir, featureName, "_as_points_wt_kss_SPO11oligos_genomeProfiles_v120619.pdf"),
    height = 4.5, width = 12)
par(mfcol = c(1, 1))
par(mar = c(4.1, 5.1, 2.1, 6.1))
par(mgp = c(3, 1, 0))
dat1Vdat2GenomePointPlot(xplot = featureTabCum$cum_midpoint,
                         dat1 = featureTabCum$wt_SPO11oligos_mean,
                         dat2 = featureTabCum$kss_SPO11oligos_mean,
                         legendNames = c("Wild type", "kyp suvh5 suvh6"),
                         plotColours = c("dodgerblue2", "darkorange1"))
dev.off()
